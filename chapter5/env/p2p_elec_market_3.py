...
class TradingSession(gym.Env):
    ...
    def __init__(self, action_space_config = 'continous', num_mutual_sessions = 3, dyn_control = 'random'):
    ...
    def reset(self):
        ...
        self.min_prices = np.array([self.session_prices.min()])
    ...
    def _update_session_prices(self):
        ...
        self.min_prices = np.hstack([self.min_prices, self.session_prices.min()])
    ...
    def _compute_optimal_strategy_reward(self):
        if self.min_prices.shape[0] < (int(self.boundary/self.constant_order)):
            return 0
        else:
            self.opt_price = np.sort(self.min_prices)[0:int(self.boundary/self.constant_order)]
            opt_quantity = np.full(int(self.boundary/self.constant_order), self.constant_order)
            return np.sum(1/np.exp(self.opt_price/self.max_session_price) - (self.opt_price/self.max_session_price)/np.exp(1))
    ...
    def _compute_reward(self, action):
        ...
        if delta_forecast_previous == delta_forecast_updated:
            ...
            self.reward = -0.1
            return self.reward
        elif self.session_prices.min() != self.session_prices[self.session_idx]:
            self.reward = 0
            return self.reward
        else:
